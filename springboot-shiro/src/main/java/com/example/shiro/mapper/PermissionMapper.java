...
public interface PermissionMapper {
    ...
    @Select( "SELECT A.NAME AS roleName,C.url FROM role AS A LEFT JOIN" +
            " permission B ON A.id=B.role_id LEFT JOIN permission AS C ON B.permission_id=C.id" +
            " where A.NAME = ${roleName}" )
    List<RolePermisson> getRolePermissionsByRoleName(String roleName);
}
